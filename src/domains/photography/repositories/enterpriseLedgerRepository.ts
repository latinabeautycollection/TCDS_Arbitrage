...
export class EnterpriseLedgerRepository {
  ...
  async deadLetter(input: {processRunId?: string; queueName: string; jobId?: string; entityType?: string; entityPk?: string; errorCode?: string; errorMessage: string; payload?: any; retryCount?: number;}) {
    await this.db.query(`INSERT INTO arb.dead_letter(process_run_id,queue_name,job_id,entity_type,entity_pk,error_code,error_message,payload_json,retry_count) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [input.processRunId ?? null,input.queueName,input.jobId ?? null,input.entityType ?? null,input.entityPk ?? null,input.errorCode ?? null,input.errorMessage,j(input.payload ?? {}),input.retryCount ?? 0]);
  }
}
