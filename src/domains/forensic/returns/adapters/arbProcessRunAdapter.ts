  ...
  async start(input: {
    ...
    const { rows } = await this.pool.query<{ run_id: string }>(
      `INSERT INTO arb.process_runs(
       ...
       ) VALUES($1,$2,'STARTED',$3,$4,$5,$6,$7,$8,$9::jsonb)
       RETURNING run_id`,
      [
        ...
        input.idempotencyKey,JSON.stringify({ tenantKey: input.principal.tenantKey }),
      ],
    );
    const row = rows[0];
    if (!row) throw new Error('Unable to create ARB process run');
    return row.run_id;
  }
