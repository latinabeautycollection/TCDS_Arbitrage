import type{Pool}from'pg';import type{CasefilePrincipal}from'../models/casefileTypes';
export class ArbProcessRunAdapter{constructor(private readonly pool:Pool){}
 async start(i:{processName:string;principal:CasefilePrincipal;correlationId:string;idempotencyKey:string;entityType:string}){
  const{rows}=await this.pool.query<{run_id:string}>(`INSERT INTO arb.process_runs(
   ...
   VALUES($1,'CASEFILE','STARTED',$2,$3,$4,$5,$6,$7) RETURNING run_id`,
  ...
  const row=rows[0];if(!row)throw new Error('Unable to create ARB process run');return row.run_id;
 }
