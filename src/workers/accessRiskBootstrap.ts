...
import { createAccessRiskWorker } from '../domains/forensic/access/workers/accessRiskWorker';
...
const worker = createAccessRiskWorker(factory as never, pool, workerPrincipal as never, `${hostname()}:${process.pid}`);
async function shutdown(sig: string): Promise<void> { logger.info({ sig }, 'access risk worker stopping'); try { await worker.close(); await pool.end(); } finally { process.exit(0); } }
process.on('SIGTERM', () => void shutdown('SIGTERM')); process.on('SIGINT', () => void shutdown('SIGINT'));
logger.info({ queue: 'domain7-access-risk' }, 'access risk worker started');
