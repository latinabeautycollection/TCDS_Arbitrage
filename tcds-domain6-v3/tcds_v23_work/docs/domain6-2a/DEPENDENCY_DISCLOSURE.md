...
dependency) and `c337eb9c67c1edc640fb25a7ce7ab093c506a05c` (certification workflow)
...
|---|---|---|---|---|---|
| `@scandit/web-datacapture-core` | 8.5.2 | dependency | Proprietary — Scandit AG. "The use of this software is governed by the Scandit Terms and Conditions" (https://ssl.scandit.com/terms/terms.pdf) | 6.2A package fragment | Scandit runtime, `DataCaptureContext`, status and loading APIs |
| `@scandit/web-datacapture-barcode` | 8.5.2 | dependency | Proprietary — Scandit AG, same terms | 6.2A package fragment | Barcode module loader registered with the context |
...
| `@types/node` | 20.19.43 | devDependency | MIT | **Added during integration — not in the 6.2A fragment** | See below |

All versions are pinned exactly. No semver ranges were introduced.
...
**Decision needed from TCDS:** keep the explicit entry (recommended), or remove it and rely on
the indirect copy.
...
packages into `public/scandit/8.5.2/` at build time and served from the application's own origin,
...
| Scope | Before 6.2A (`9846d0c`) | After 6.2A (audited 2026-09-14; dependencies unchanged since) |
|---|---|---|
