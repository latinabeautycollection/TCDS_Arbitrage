import { AlertTriangle, CheckCircle2, CloudOff, PackageOpen } from 'lucide-react';

export function LoadingSkeleton({ label = 'Loading warehouse data…' }: { label?: string }) {
  return (
    <div className="rounded-enterprise border border-tcds-line bg-white p-4 shadow-soft" aria-label={label}>
      <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-tcds-muted">
        <span className="h-2 w-2 rounded-full bg-tcds-gold" /> {label}
      </div>
...
export function EmptyState({ title, message }: { title: string; message: string }) {
  return (
    <div className="rounded-enterprise border border-dashed border-tcds-strongLine bg-white p-5 text-center shadow-soft">
...
export function ErrorState({ title, message }: { title: string; message: string }) {
  return (
    <div className="rounded-enterprise border border-tcds-red/20 bg-red-50 p-4 shadow-soft">
...
export function OfflineState() {
  return (
    <div className="rounded-enterprise border border-tcds-warning/25 bg-amber-50 p-4 shadow-soft">
...
export function SuccessToastPreview({ message }: { message: string }) {
  return (
    <div className="rounded-enterprise border border-tcds-green/20 bg-emerald-50 p-4 shadow-soft">
      <div className="flex items-center gap-3">
